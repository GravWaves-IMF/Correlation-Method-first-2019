...
def get_residual_strain(tev,window):
    ...
    for ifo in ifos:
        ...
        strain[ifo] = strain[ifo].time_slice(tev-window/2, tev+window/2)
    return strain
...
def get_strain(pth,tev,window=512):
...
def get_NoNan_strain(pth,tev,tstart,tend,window):
    """This function, require the strain .gwf name, without the detector definition 
    (H-H1,L-L1), and loads the entire frame file downloaded from GW OSC,
    applies a high-pass filter at 15 Hz, then keeps the data around
    ``event_time`` +/-``window``. The ``event_time`` is from above; the
...
def get_LSC_Full_strain(pth,tstart,tend):
    """This function, require the strain .gwf name, without the detector definition 
    (H-H1,L-L1), and loads the entire frame file downloaded from GW OSC,
...
def bandpass_wv(strain, flow, fhigh, bw=3, method='losc'):
    ts = {}
    if method=='fir':
        for ifo in ifos:
            ts[ifo] = strain[ifo].lowpass_fir(fhigh, order).highpass_fir(flow, order)
...
def whiten_wv(strain, flow, fhigh):
    ts = {}
    for ifo in ifos:
        ts[ifo] = strain[ifo].crop(8, 8).whiten(32, 32, trunc_method=None,
                                                        low_frequency_cutoff=15.0)
...
def indices_within_window(corr, timedl, tdlerr):
    ...
    l = numpy.searchsorted(times, timedl - tdlerr, side='right') - 1
    r = numpy.searchsorted(times, timedl + tdlerr, side='left') + 1
    return l, r, times[l], times[r-1] 


def corr_near_ml(corr, timedl, tdlerr, width=time_uncertainty):
    """Get the value of the peak correlation within the expected time offset
    region.
    """
    l, r, start, end = indices_within_window(corr, timedl, tdlerr)
    return abs(corr[l:r]).max()

def corr_wsgn_near_ml(corr, timedl, tdlerr, width=time_uncertainty):
    """Get the value of the peak correlation within the expected time offset
    region, it report the value of the peak with sign.
    """
    l, r, start, end = indices_within_window(corr, timedl, tdlerr)
    corrmax = (corr[l:r]).max()
    corrmin = (corr[l:r]).min() 
    if (abs(corrmin) > corrmax):
    	corrmax = corrmin
    return corrmax

def get_peak(strain, time, timedl, tdlerr,  width=time_uncertainty):
    ...
    peak = corr_near_ml(corr, timedl, tdlerr)
    return peak
